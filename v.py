...
def guessing_game_by_user():
    if 'num_to_be_guessed' not in st.session_state:
        st.session_state.num_to_be_guessed = random.randint(1, 100)
    ...
    if st.session_state.count < 11:
        guessed_number = st.number_input(label='ENTER YOUR GUESSED NUMBER between 1 to 100:', key='num')
...
def guessing_game_by_machine():
    ...
    if st.session_state.attempt < attempts:
        ...
        response = st.radio("Please select:", ('Correct value', 'High value ', 'Low value'))
        if st.checkbox('submit'):
            if response == 'Correct value':
            ...
            elif response == 'High value':
                st.session_state.high = mid - 1
            elif response == 'Low value':
...
if st.session_state.select == 0:
    ...
    """)
    st.write("email: a.madhumithaarunkumar@gmail.com")
